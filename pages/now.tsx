        ...
        {showSecret && (
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4 mb-8 animate-pulse">
            <p className="text-green-800 dark:text-green-200 text-sm">
              You found all the easter eggs! Here's a secret: I once accidentally brought down production by running data mutation commands on traffic taking node, we all are humans at the end of the day.
            </p>
          </div>
        )}
      </header>
      ...
      <main className="space-y-12">
        ...
        <section>
          ...
          <p className="text-gray-700 dark:text-gray-300">
            <span className="font-medium">The Psychology of Money:</span> Timeless Lessons on Wealth, Greed, and Happiness
          </p>
        </section>
        ...
        <section>
          ...
          <div className="space-y-2 text-gray-700 dark:text-gray-300">
            {trips.map((trip, index) => (
              <div key={index} className="flex items-center gap-2">
                <span className={trip.completed ? "text-green-600 dark:text-green-400" : "text-gray-400 dark:text-gray-600"}>
                  {trip.completed ? "✓" : "○"}
                </span>
                <span className={trip.completed ? "line-through opacity-75" : ""}>
                  {trip.name}
                </span>
                {trip.completed && (
          ...
          <div className="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
            <p className="text-sm text-blue-800 dark:text-blue-200 leading-relaxed">
              <span className="font-semibold">Tech Setup:</span> I have set up my own NFS @ home, running on a minimal dual-core Intel CPU + 4TB on ZFS mirror (effectively 2TB). Each week, all photos from my iCloud are automatically transferred and categorized into year/month folders.
